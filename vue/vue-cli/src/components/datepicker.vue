<template>
	<div class="vue-datepicker">
		...
		<div v-show="show" class="vue-datepicker-wrap">
			...
			<div class="vue-datepicker-content">
				<table>
					...
					<tbody>
						<tr v-for="(week,i) of list" :key="i">
							<td v-for="(weekday,j) of week" @click="pick(weekday)"
								:class="{
								'flag': weekday.flag, 
								'active': !weekday.flag && weekday.text == current.date && select.month == current.month && select.year == current.year}" 
								:key="j"
							>
								{{weekday.text}}
							</td>
				...
				<div v-show="selectYear" class="vue-date-picker-year-panel">
					<ul ref="year">
						<li v-for="(y,index) of years" :key="index" @click.stop="pickYear(y)" :class="{'active': y == select.year}">{{y}}</li>
					</ul>
					<ul ref="month">
...
.vue-datepicker .vue-datepicker-wrap .vue-datepicker-header .vue-datepicker-header-btn-pre,.vue-datepicker .vue-datepicker-wrap .vue-datepicker-header .vue-datepicker-header-btn-next{
	padding: 0 15px;
}
